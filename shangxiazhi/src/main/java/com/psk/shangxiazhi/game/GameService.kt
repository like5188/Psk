...
@OptIn(DelicateCoroutinesApi::class, KoinApiExtension::class)
class GameService : Service(), KoinComponent {
    companion object {
    ...
    fun start(
        ...
        val gameCallback = object : GameCallback.Stub() {
            ...
            override fun onLoading() {
                ...
                bleManager.connectAll(GlobalScope, 3000L, onConnected = {
                    when (it.type) {
    ...
    private fun fetchShangXiaZhiAndSave() {
        ...
        fetchShangXiaZhiAndSaveJob = GlobalScope.launch(Dispatchers.IO) {
    ...
    private fun fetchHeartRateAndSave() {
        ...
        fetchHeartRateAndSaveJob = GlobalScope.launch(Dispatchers.IO) {
    ...
    private fun fetchBloodOxygenAndSave() {
        ...
        fetchBloodOxygenAndSaveJob = GlobalScope.launch(Dispatchers.IO) {
            while (isActive) {
    ...
    private fun fetchBloodPressureAndSave() {
        ...
        fetchBloodPressureAndSaveJob = GlobalScope.launch(Dispatchers.IO) {
            while (isActive) {
